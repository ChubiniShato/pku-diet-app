...
public interface MealSlotRepository extends JpaRepository<MealSlot, UUID> {
    ...
    @Query("SELECT ms FROM MealSlot ms WHERE ms.menuDay.patient.id = :patientId AND ms.isConsumed = :consumed")
    List<MealSlot> findByPatientIdAndConsumed(@Param("patientId") UUID patientId, @Param("consumed") Boolean consumed);

    @Query("SELECT ms FROM MealSlot ms WHERE ms.menuDay.id = :menuDayId AND ms.actualPheMg > ms.targetPheMg")
