...
public class MenuController {
    ...
    @Operation(summary = "Get menu weeks for a patient", description = "Retrieve all menu weeks for a specific patient")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved menu weeks",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MenuWeekDto.class)))
    })
    ...
    @Operation(summary = "Get menu week by ID", description = "Retrieve a specific menu week by its unique identifier")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved menu week",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MenuWeekDto.class))),
    ...
    public ResponseEntity<MenuWeekDto> getMenuWeekById(
            @Parameter(description = "Menu week unique identifier", required = true)
    ...
    @Operation(summary = "Create new menu week", description = "Create a new weekly menu for a patient")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Menu week created successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MenuWeekDto.class))),
    ...
    public ResponseEntity<MenuWeekDto> createMenuWeek(
            @Parameter(description = "Menu week creation request", required = true)
            @Valid @RequestBody CreateMenuWeekRequest request) {
    ...
    @Operation(summary = "Delete menu week", description = "Delete a menu week and all associated daily menus")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Get menu days for a week", description = "Retrieve all daily menus for a specific week")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved menu days",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MenuDayDto.class)))
    })
    ...
    @Operation(summary = "Get menu day by ID", description = "Retrieve a specific daily menu by its unique identifier")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved menu day",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MenuDayDto.class))),
    ...
    public ResponseEntity<MenuDayDto> getMenuDayById(
            @Parameter(description = "Menu day unique identifier", required = true)
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Menu day created successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MenuDayDto.class))),
            @ApiResponse(responseCode = "400", description = "Invalid request data")
    })
    @PostMapping("/days")
    public ResponseEntity<MenuDayDto> createMenuDay(
            @Parameter(description = "Menu day creation request", required = true)
    ...
    @Operation(summary = "Delete menu day", description = "Delete a daily menu and all associated meal slots")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Get meal slots for a day", description = "Retrieve all meal slots for a specific daily menu")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved meal slots",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MealSlotDto.class)))
    })
    ...
    @Operation(summary = "Get menu entries for a slot", description = "Retrieve all menu entries for a specific meal slot")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved menu entries",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MenuEntryDto.class)))
    })
    @GetMapping("/slots/{slotId}/entries")
    public ResponseEntity<List<MenuEntryDto>> getMenuEntriesBySlot(
            @Parameter(description = "Meal slot unique identifier", required = true)
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Menu entry added successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MenuEntryDto.class))),
            @ApiResponse(responseCode = "400", description = "Invalid request data"),
    ...
    public ResponseEntity<MenuEntryDto> addMenuEntry(
            @Parameter(description = "Meal slot unique identifier", required = true)
            @PathVariable UUID slotId,
            @Parameter(description = "Menu entry creation request", required = true)
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Menu entry updated successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MenuEntryDto.class))),
    ...
    public ResponseEntity<MenuEntryDto> updateMenuEntry(
            @Parameter(description = "Menu entry unique identifier", required = true)
            @PathVariable UUID entryId,
            @Parameter(description = "Menu entry update request", required = true)
            @Valid @RequestBody UpdateMenuEntryRequest request) {
    ...
    @Operation(summary = "Mark entry as consumed", description = "Mark a menu entry as consumed or not consumed")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Menu entry consumption status updated successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = MenuEntryDto.class))),
            @ApiResponse(responseCode = "404", description = "Menu entry not found")
    })
    ...
    public ResponseEntity<MenuEntryDto> markEntryAsConsumed(
            @Parameter(description = "Menu entry unique identifier", required = true)
            @PathVariable UUID entryId,
            @Parameter(description = "Consumed status", required = true)
