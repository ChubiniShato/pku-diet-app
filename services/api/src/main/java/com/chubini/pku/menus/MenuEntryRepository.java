...
public interface MenuEntryRepository extends JpaRepository<MenuEntry, UUID> {
  ...
  @Query(
      "SELECT me FROM MenuEntry me WHERE me.mealSlot.menuDay.patient.id = :patientId AND me.entryType = :entryType")
  List<MenuEntry> findByPatientIdAndEntryType(
      @Param("patientId") UUID patientId, @Param("entryType") MenuEntry.EntryType entryType);
