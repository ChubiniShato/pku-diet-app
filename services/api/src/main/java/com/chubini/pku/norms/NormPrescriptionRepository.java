...
public interface NormPrescriptionRepository extends JpaRepository<NormPrescription, UUID> {
  ...
  @Query(
      "SELECT n FROM NormPrescription n WHERE n.patient.id = :patientId ORDER BY n.prescribedDate DESC")
  List<NormPrescription> findByPatientIdOrderByDate(@Param("patientId") UUID patientId);

  @Query(
      "SELECT COUNT(n) FROM NormPrescription n WHERE n.patient.id = :patientId AND n.isActive = true")
  long countActiveByPatientId(@Param("patientId") UUID patientId);

  @Query(
      "SELECT n FROM NormPrescription n WHERE n.isActive = true AND n.pheLimitMgPerDay BETWEEN :minPhe AND :maxPhe")
  List<NormPrescription> findActiveByPheLimitRange(
      @Param("minPhe") Double minPhe, @Param("maxPhe") Double maxPhe);

  List<NormPrescription> findByPatientIdAndIsActiveTrue(UUID patientId);

  Optional<NormPrescription> findTopByPatientIdAndIsActiveTrueOrderByPrescribedDateDesc(
      UUID patientId);
}
