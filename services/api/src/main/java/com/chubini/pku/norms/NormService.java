...
public class NormService {
  ...
  public List<NormPrescriptionDto> getNormsByPatient(UUID patientId) {
    ...
    List<NormPrescription> norms =
        normRepository.findByPatientIdOrderByPrescribedDateDesc(patientId);
  ...
  public Optional<NormPrescriptionDto> getCurrentNormForPatient(UUID patientId) {
    ...
    Optional<NormPrescription> currentNorm =
        normRepository.findTopByPatientIdAndIsActiveTrueOrderByPrescribedDateDesc(patientId);
  ...
  public NormPrescriptionDto getNormById(UUID normId) {
    ...
    NormPrescription norm =
        normRepository
            .findById(normId)
            .orElseThrow(
                () -> new NormNotFoundException("Norm prescription not found with ID: " + normId));
  ...
  private void deactivateOtherNormsForPatient(UUID patientId) {
    ...
    if (!activeNorms.isEmpty()) {
      ...
      log.debug(
          "Deactivated {} existing active norms for patient: {}", activeNorms.size(), patientId);
    }
  }
}
