...
public interface PatientProfileRepository extends JpaRepository<PatientProfile, UUID> {
    ...
    Page<PatientProfile> findByNameContainingIgnoreCase(@Param("name") String name, Pageable pageable);

    /**
     * Find patients by birth date range
     */
    @Query("SELECT p FROM PatientProfile p WHERE p.birthDate BETWEEN :startDate AND :endDate")
    List<PatientProfile> findByBirthDateBetween(@Param("startDate") LocalDate startDate, 
    ...
    Page<PatientProfile> findByMultipleCriteria(@Param("name") String name,
